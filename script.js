...
function updateChart(data) {
  ...
  d3.select("#bar").selectAll("svg").remove(); //clean slate for updating chart
  createVerticalDivergingBarChart(newData, maxAbs); //plot
  window.addEventListener("resize", () => { 
    updateChart(data); 
    });// or whatever your data variable is });
  if (data.length === 0) { d3.select("#bar").html("<p>No data available.</p>"); return; }
}
...
const createVerticalDivergingBarChart = (data, maxAbs) => {
  ...
  // const svg = d3.select("#bar") // point everything of this function to id bar in html
  //   .append("svg")
  //   .attr("width", "100%") 
  //   .attr("height", "400px")
  //   ;
...
// function updateCountrySelect(continent) {
//   const countrySelect = document.getElementById("select_country");
//   countrySelect.innerHTML = ""; // clear old options
